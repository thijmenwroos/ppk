...
class RegressionPlot(ModelPlot, ABC):
    ...
    def getSummary(self):
        ...
        print(df.head())
        masked_r2 = MaskedMetric(metric=SklearnMetrics("r2"))
        masked_rmse = MaskedMetric(metric=SklearnMetrics("neg_mean_squared_error"))
        df_summary = (
            df.groupby(["Model", "Fold", "Property"])
            .apply(
                lambda x: pd.Series(
                    {
                        "R2": masked_r2(x["Label"], x["Prediction"]),
                        "RMSE": -1*masked_rmse(
...
class CorrelationPlot(RegressionPlot):
    ...
    def make(
        ...
        if not hasattr(self, "summary"):
            self.getSummary()
        for key, grp in df.groupby("Property"):
            # plot the results
            summ = self.summary[self.summary["Property"] == key]
            g = sns.FacetGrid(
                grp,
                col="Property",
                row="Model",
            ...
            g.add_legend()
            table = plt.table(
                cellText=(summ.get(['R2', 'RMSE']).to_numpy()).round(decimals=2),
                rowLabels=summ.get(['Fold']).to_numpy(),
                colLabels=['R2', 'RMSE'],
                colWidths=[0.07, 0.07],
                bbox=(1.4, 0.0, 0.25, 0.3))


        # save the plot
            if save:
                if out_path is not None:
                    plt.savefig(out_path + f"{key}_correlation.png", dpi=300)
                else:
